...
export class NPCParser {
  static MEGA_NPCS_PATH = '[2] 🔮 Matrizes do VTT/MegaNPCs.md';
  ...
  static async loadCategories(): Promise<NPCCategory[]> {
    ...
    try {
      ...
      console.error('Erro no NPCParser:', error);
      return [];
    }
  }

  static async getCategories(): Promise<NPCCategory[]> {
    if (this.cachedCategories) return this.cachedCategories;

...
export const ConspiracyBoardWidget: React.FC<ConspiracyBoardWidgetProps> = ({ onClose }) => {
  const [board, _setBoard] = useState<MindMapData>({
    id: 'default',
  ...
  const centerCanvas = () => updateBoard(b => ({ ...b, offsetX: 0, offsetY: 0, zoom: 1 }));
  const changeZoom = (delta: number) =>
    updateBoard(b => ({ ...b, zoom: Math.max(0.15, Math.min(b.zoom + delta, 4)) }));
  ...
  const addNode = (type: MindMapNode['type'], extraProps: Partial<MindMapNode> = {}) => {
    ...
    updateBoard(b => ({ ...b, nodes: [...b.nodes, newNode] }));
  };
  ...
  const updateNode = (id: string, changes: Partial<MindMapNode>) =>
    updateBoard(b => ({ ...b, nodes: b.nodes.map(n => (n.id === id ? { ...n, ...changes } : n)) }));

  // --- MOUSE HANDLERS ---
  const handleMouseMove = (e: React.MouseEvent) => {
    if (isPanning) {
      updateBoard(b => ({ ...b, offsetX: b.offsetX + e.movementX, offsetY: b.offsetY + e.movementY }));
      return;
    }
    if (draggingNode) {
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: board.bgColor, fontFamily: 'var(--font-body)', color: 'var(--text-primary)', position: 'relative' }}>
        ...
        <div style={{
          ...
          display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 14px', flexWrap: 'wrap', justifyContent: 'center', maxWidth: '95vw', width: 'max-content',
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                  ...
                  <input
                    ...
                    onChange={e => updateBoard(b => ({ ...b, bgColor: e.target.value }))}
                  />
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      updateBoard(bd => ({ ...bd, gridColor: `rgba(${r},${g},${b},0.15)` }));
                    }}
                  />
                </div>
              </div>
          ...
          <input
            value={board.name}
            onChange={e => updateBoard(b => ({ ...b, name: e.target.value.replace(/[^a-zA-Z0-9_\-\s]/g, '') }))}

...
import { Activity, BookOpen, Crown, DoorOpen, Download, Eye, FileJson, Image, LayoutDashboard, Lock, Menu, Pencil, Plus, Printer, ScrollText, Shield, Swords, Trash2, Upload, Unlock, UserCog, UserPlus, Users, WandSparkles, X } from 'lucide-react';
...
export function AdminCommandCenter({ roomCode, scope = 'platform' }: { roomCode: string; scope?: 'platform' | 'campaign' }) {
  ...
  const deleteTable = async (campaign: CampaignCloudRecord) => {
    if (!user?.id) return;
    if (window.confirm(`Excluir definitivamente a mesa “${campaign.name}”? Todos os dados e registros locais desta mesa serão removidos.`)) {
      setCampaigns(prev => prev.filter(c => c.id !== campaign.id && c.room_code !== campaign.room_code));
      await deleteCampaignCloud(campaign.id, user.id);
      setNotice(`Mesa “${campaign.name}” excluída definitivamente.`);
      await reload();
    }
  };
  const toggleLock = async (campaign: CampaignCloudRecord) => {
    if (!user?.id) return;
    const nextClosed = !campaign.is_closed;
    await toggleCampaignLock(campaign.id, nextClosed, user.id);
    setNotice(`Mesa “${campaign.name}” agora está ${nextClosed ? 'trancada (bloqueada)' : 'aberta'}.`);
    await reload();
  };
    ...
    <section className="command-content">
      ...
      <main>
        {scope === 'platform' && platformAllowed === false ? <div className="command-notice" role="alert">Acesso reservado às contas administradoras da plataforma.</div> : scope === 'platform' && platformAllowed === null ? <div className="command-notice" role="status">Confirmando suas permissões administrativas…</div> : <>
          ...
          {view === 'tables' ? <Tables campaigns={campaigns} selectedId={selectedCampaign?.id} onSelect={selectCampaign} onSave={saveTable} onDelete={deleteTable} onToggleLock={toggleLock} /> : null}
...
function Tables({ campaigns, selectedId, onSelect, onSave, onDelete, onToggleLock }: { campaigns: CampaignCloudRecord[]; selectedId?: string; onSelect: (id: string, view?: View) => void; onSave: (draft: TableDraft, campaign?: CampaignCloudRecord) => Promise<void>; onDelete: (campaign: CampaignCloudRecord) => Promise<void>; onToggleLock: (campaign: CampaignCloudRecord) => Promise<void> }) {
  ...
  return <div className="command-table-layout"><section className="command-panel command-table"><header><div><span>GESTÃO</span><h3>Mesas do Reino</h3></div><button onClick={() => setEditingId('new')}><Plus size={15} />Nova mesa</button></header><div className="command-table-head"><span>Mesa</span><span>Visibilidade</span><span>Status</span><span>Ações</span></div>{campaigns.map(campaign => <div className={`command-table-row ${selectedId === campaign.id ? 'is-selected' : ''}`} key={campaign.id}><div><strong>{campaign.name}</strong><small>{campaign.system} · {campaign.room_code}</small></div><span>{campaign.is_public ? 'Mural público' : 'Só por convite'}</span><span className={campaign.is_closed ? 'badge muted' : 'badge'}>{campaign.is_closed ? 'Bloqueada' : 'Aberta'}</span><div className="command-row-actions"><button onClick={() => { onSelect(campaign.id); setEditingId(campaign.id); }} title="Mudar nome e configurações"><Pencil size={14} />Configurar</button><button onClick={() => void onToggleLock(campaign)} title={campaign.is_closed ? 'Destrancar mesa' : 'Trancar mesa'}>{campaign.is_closed ? <Unlock size={14} /> : <Lock size={14} />}{campaign.is_closed ? 'Destrancar' : 'Trancar'}</button><button className="danger" onClick={() => void onDelete(campaign)} title="Excluir mesa definitivamente"><Trash2 size={14} />Excluir</button><a href={`/vtt.html?room=${encodeURIComponent(campaign.room_code)}`}>Abrir</a></div></div>)}</section>{editingId ? <TableEditor key={editingId} campaign={editing} initial={initial} onCancel={() => setEditingId(null)} onSave={async draft => { await onSave(draft, editing); setEditingId(null); }} onDelete={editing ? async () => { await onDelete(editing); setEditingId(null); } : undefined} /> : null}</div>;
}

function TableEditor({ campaign, initial, onCancel, onSave, onDelete }: { campaign?: CampaignCloudRecord; initial: TableDraft; onCancel: () => void; onSave: (draft: TableDraft) => Promise<void>; onDelete?: () => Promise<void> }) {

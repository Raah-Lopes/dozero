...
function buildFsTree(dir, baseDir, tree = []) {
...
function buildGraph(dir, baseDir, nodes = [], links = []) {
    if (!fs.existsSync(dir))
        return { nodes, links };
    const files = fs.readdirSync(dir);
    for (const file of files) {
        if (file === '.git' || file === 'node_modules' || file === 'ANEXOS')
            continue;
        const fullPath = path.join(dir, file);
        const stat = fs.statSync(fullPath);
        if (stat.isDirectory()) {
            ...
            server.middlewares.use(async (req, res, next) => {
                ...
                try {
                    ...
                    if (req.method === 'GET' && pathname === '/api/wiki/file') {
                        ...
                        if (!filepath)
                            return sendResponse(400, { error: 'path required' });
                        const full = path.join(repoPath, filepath);
                        if (!fs.existsSync(full))
                            return sendResponse(404, { error: 'file not found' });
                    ...
                    if (req.method === 'GET' && pathname === '/api/wiki/search') {
                        ...
                        if (!query)
                            return sendResponse(400, { error: 'query required' });
                        const results = [];
                    ...
                    if (req.method === 'POST' && pathname === '/api/wiki/init') {
                        ...
                        if (!fs.existsSync(templatePath))
                            return sendResponse(500, { error: 'Template folder not found' });
                    ...
                    if (req.method === 'POST' && pathname === '/api/wiki/save') {
                        ...
                        if (!filepath || content === undefined)
                            return sendResponse(400, { error: 'path and content required' });
                        const full = path.join(repoPath, filepath);
                    ...
                    if (req.method === 'POST' && pathname === '/api/wiki/move') {
                        ...
                        if (!oldPath || !newPath)
                            return sendResponse(400, { error: 'oldPath and newPath required' });
                    ...
                    if (req.method === 'POST' && pathname === '/api/wiki/rename') {
                        ...
                        if (!oldPath || !newName || !repo)
                            return sendResponse(400, { error: 'Missing parameters' });
                        ...
                        function updateLinksRecursive(directory) {
                            ...
                            for (const file of files) {
                                ...
                                if (stat.isDirectory()) {
                                    ...
                                    const newContent = content.replace(regex, (match, inner) => {
                                        ...
                                        if (cleanTarget.toLowerCase().endsWith('.md'))
                                    ...
                                    if (modified)
                                        fs.writeFileSync(fullP, newContent);
                                }
                            }
                        }
                    ...
                    if (req.method === 'POST' && pathname === '/api/wiki/save-image') {
                        ...
                        if (!filename || !base64)
                            return sendResponse(400, { error: 'filename and base64 required' });
